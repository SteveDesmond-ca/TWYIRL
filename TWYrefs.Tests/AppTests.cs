...
public class AppTests
{
    ...
    public async Task SkipsBlankLines()
    {
        //arrange
        var fs = Substitute.For<IFileSystem>();
        fs.File.ReadAllLinesAsync(Arg.Any<string>()).Returns(new[] { "", "lyrics 456" });
    ...
    [Fact]
    public async Task SkipsTitles()
    {
        //arrange
        var fs = Substitute.For<IFileSystem>();
        fs.File.ReadAllLinesAsync(Arg.Any<string>()).Returns(new[] { "# Song Name", "lyrics 456" });
    ...
    [Fact]
    public async Task FailsWhenNothingNew()
    {
        //arrange
        var fs = Substitute.For<IFileSystem>();
        fs.File.ReadAllLinesAsync(Arg.Any<string>()).Returns(new[] { "lyrics" });
        ...
        //assert
        Assert.Contains("No new lyrics", console.ToString());
        Assert.Equal(1, result);
    }
}